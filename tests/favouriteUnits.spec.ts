...
async function createUnits(unitApiHelper: any, unitsCount: number) {
    ...
    test.describe("Favourite units functionality check", async () => {
        ...
        test("Verify 'Обрані' icon functionality", async ({ unitApiHelper, myTenders, navBar, loginPopUp, telegramPopUp, unitsPage, myFavouriteUnitsPage, myUnitsPage, ownCabinetLeftSideMenu }) => {
            const unitsCount = 1;
            ...
            await navBar.clickElement('avatarIcon');
            await navBar.clickProfileDropdownBtn('advertisments');
            await expect(myUnitsPage.getTitle()).toHaveText(myAdvertismentsData.title);
            await telegramPopUp.closeTelegramPopUp();
            await ownCabinetLeftSideMenu.clickSelectedAdvertismentsItem();
